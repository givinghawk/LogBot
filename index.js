...
client.on('message', message => {
    ...
    if (message.content == prefix + 'download' && message.member.id == message.guild.ownerID) {
        ...
        fetch('http://51.195.166.36:1045/documents', {
            method: 'POST',
            body: logs
        }).then(res => res.json()).then(json => {
            // send the hastebin link to the channel
            message.channel.send('http://51.195.166.36:1045/' + json.key);
        });
    }
    ...
    if (message.content == prefix + 'dump' && message.author.id == config.ownerid) {
        ...
        fetch('http://51.195.166.36:1045/documents', {
            ...
            message.author.send('http://51.195.166.36:1045/' + json.key);
        });
    }
...
// start a web server on port 6969
